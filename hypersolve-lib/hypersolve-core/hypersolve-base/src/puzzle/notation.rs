...
fn parse_standard_twist_string(mut s: &str) -> Result<Twist, ParseStandardTwistError> {
    ...
    // get the direction
    let face_vec = dir_faces
        .into_iter()
        .map(|f| f.into_vector4())
        .reduce(|acc, v| [0, 1, 2, 3].map(|i| acc[i] + v[i]))
        .unwrap();
    let mut direction =
        TwistDirection::from_signs_within_face([0, 1, 2].map(|i| dot(&face_vec, &face.basis()[i])))
            .unwrap();
...
fn mc4d_twist_order() -> impl Iterator<Item = Option<(Face, TwistDirection)>> {
    ...
    let corners = piece_locations
        .clone()
        .filter(|v| v.iter().map(|i| i * i).sum::<i32>() == 3);
    let edges = piece_locations
        .clone()
        .filter(|v| v.iter().map(|i| i * i).sum::<i32>() == 2);
    let centers = piece_locations.filter(|v| v.iter().map(|i| i * i).sum::<i32>() == 1);
    ...
    MC4D_FACE_ORDER.into_iter().flat_map(move |face| {
        ...
        basis.sort_by_key(|f| f.axis() as u8); // order: X, Y, Z, W
        ...
        mc4d_order_piece_locations
            ...
            .map(move |mc4d_coords_of_sticker_within_face: [i32; 3]| {
                ...
                TwistDirection::from_signs_within_face(signs_within_face(face, offset.map(|i| -i)))
            })
            .map(move |twist_dir| Some((face, twist_dir?)))
    })
}

fn dot<const N: usize>(vec1: &[i32; N], vec2: &[i32; N]) -> i32 {
    vec1.iter().zip(vec2.iter()).map(|(a, b)| a * b).sum()
}

fn signs_within_face(face: Face, piece_loc_signs: [i32; 4]) -> [i32; 3] {
    let [basis1, basis2, basis3] = face.basis();
    [
        dot(&piece_loc_signs, &basis1),
        dot(&piece_loc_signs, &basis2),
        dot(&piece_loc_signs, &basis3),
    ]
}
...
mod tests {
    ...
    fn test_twist_to_from_standard_string() {
        Twist::ALL_TWISTS.into_iter().for_each(|twist| {
            assert_eq!(
                twist,
